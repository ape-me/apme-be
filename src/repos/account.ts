...
export const accountRepo = {
  touch: (sql: Sql, id: string, t: number, emailHash: string | null) =>
    sql<UserRow[]>`UPDATE users SET last_seen_at = ${t}, email_hash = COALESCE(${emailHash}, email_hash) WHERE id = ${id} RETURNING *`,
  create: (sql: Sql, id: string, emailHash: string | null, code: string, t: number) =>
    sql<UserRow[]>`INSERT INTO users (id, email_hash, referral_code, created_at, last_seen_at) VALUES (${id}, ${emailHash}, ${code}, ${t}, ${t}) RETURNING *`,
  ...
  createSettings: (sql: Sql, id: string, t: number) => sql`INSERT INTO user_settings (user_id, updated_at) VALUES (${id}, ${t}) ON CONFLICT DO NOTHING`,
  ...
  updateProfile: (sql: Sql, id: string, handle: string | null, avatar: string | null | undefined) =>
    sql<UserRow[]>`UPDATE users SET handle = COALESCE(${handle}, handle), avatar_url = ${avatar === undefined ? sql`avatar_url` : avatar} WHERE id = ${id} RETURNING *`,

  wallets: (sql: Sql, userId: string) => sql<WalletRow[]>`SELECT * FROM wallets WHERE user_id = ${userId} ORDER BY hd_index`,
  addWallet: (sql: Sql, w: { address: string; userId: string; chain: string; idx: number; label: string; isDefault: boolean; t: number }) =>
  ...
  setDefaultWallet: (sql: Sql, userId: string, address: string) => sql`UPDATE wallets SET is_default = (address = ${address}) WHERE user_id = ${userId}`,
  labelWallet: (sql: Sql, address: string, label: string) => sql`UPDATE wallets SET label = ${label} WHERE address = ${address}`,

  settings: (sql: Sql, userId: string) => sql<SettingsRow[]>`SELECT * FROM user_settings WHERE user_id = ${userId}`,
  updateSettings: (sql: Sql, userId: string, s: { slippageBps: number; buyCsv: string; sellCsv: string; priority: string; confirm: boolean; hideDust: boolean }) =>
    sql<SettingsRow[]>`UPDATE user_settings SET slippage_bps = ${s.slippageBps}, quick_buy_usd = string_to_array(${s.buyCsv}, ',')::int[],
  ...
  redeem: (sql: Sql, code: string, ownerId: string | null, userId: string, t: number) => sql.begin(async (tx) => {
    const bumped = await tx`UPDATE invite_codes SET uses = uses + 1 WHERE code = ${code} AND uses < max_uses RETURNING code`;
    if (!bumped.length) return false;
    await tx`UPDATE users SET activated_at = ${t}, invited_by = ${ownerId} WHERE id = ${userId}`;
    if (ownerId) await tx`INSERT INTO referrals (referee_user_id, referrer_user_id, code, created_at) VALUES (${userId}, ${ownerId}, ${code}, ${t}) ON CONFLICT DO NOTHING`;
    return true;
  }),
  listInvites: (sql: Sql) => sql`SELECT i.code, i.kind, i.label, i.max_uses, i.uses, i.expires_at, i.created_at, u.handle AS owner
                                 FROM invite_codes i LEFT JOIN users u ON u.id = i.owner_user_id ORDER BY i.created_at DESC LIMIT 500`,
  createAdminCode: (sql: Sql, code: string, maxUses: number, label: string | null, expiresAt: number | null, t: number) =>
  ...
  referred: (sql: Sql, referrerId: string) => sql<{ id: string; handle: string | null; joined_at: number; volume_usd: number | null }[]>`
