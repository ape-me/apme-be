...
export type StockPatch = { excluded?: boolean; category?: string | null; tags?: string[]; note?: string | null };
...
export async function patchStock(sql: Sql, mint: string, b: StockPatch) {
  ...
  const next: StockConfig = { excluded: b.excluded ?? cur?.excluded ?? false, category: b.category === undefined ? cur?.category ?? null : b.category, tags: b.tags ?? pgArr(cur?.tags), note: b.note === undefined ? cur?.note ?? null : b.note };
  ...
  return { ok: true, config: { mint, ...next }, stock, note: "indexer applies subscription changes within 10 min" };
}
