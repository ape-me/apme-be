...
export async function wallet(env: Env, sql: Sql, address: string, activityLimit: number): Promise<z.infer<typeof WalletResponse>> {
  ...
  const add = (mint: string, b: { boughtRaw: number; boughtUsd: number; soldRaw: number; soldUsd: number }) => {
    const cur = basis.get(mint) ?? { boughtRaw: 0, boughtUsd: 0, soldRaw: 0, soldUsd: 0 };
    basis.set(mint, { boughtRaw: cur.boughtRaw + b.boughtRaw, boughtUsd: cur.boughtUsd + b.boughtUsd, soldRaw: cur.soldRaw + b.soldRaw, soldUsd: cur.soldUsd + b.soldUsd });
  };
  for (const p of tradePositions) add(p.token_mint, { boughtRaw: Number(p.bought_raw), boughtUsd: p.bought_usd, soldRaw: Number(p.sold_raw), soldUsd: p.sold_usd });
  for (const p of swapPositions) add(p.mint, { boughtRaw: Number(p.bought_raw), boughtUsd: Number(p.bought_usd), soldRaw: Number(p.sold_raw), soldUsd: Number(p.sold_usd) });
  ...
  holdings.push({ mint: USDC_MINT, kind: "cash", symbol: "USDC", name: "Cash", image: USDC_LOGO, quoteSymbol: null, amount: cashUsd, raw: usdc?.raw ?? "0", decimals: 6, priceUsd: 1, valueUsd: round(cashUsd), change24h: null, costUsd: null, pnlUsd: null, pnlPct: null });
  const solUsd = solUsdPrice == null ? 0 : sol * solUsdPrice;
  if (sol > 0) holdings.push({ mint: SOL_MINT, kind: "sol", symbol: "SOL", name: "Solana", image: SOL_LOGO, quoteSymbol: null, amount: sol, raw: String(Math.round(sol * 1e9)), decimals: 9, priceUsd: round(solUsdPrice), valueUsd: round(solUsd), change24h: null, costUsd: null, pnlUsd: null, pnlPct: null });
  ...
  for (const t of tokens) {
    ...
    holdings.push({
      ...
      amount, raw: t.raw, decimals: t.decimals, priceUsd: m.price_usd, valueUsd: round(value), change24h: m.change_24h,
      costUsd: round(cost), pnlUsd: round(pnl), pnlPct: round(pnlPct),
    });
  }
  holdings.sort((a, b) => (a.kind === "cash" ? -1 : b.kind === "cash" ? 1 : (b.valueUsd ?? 0) - (a.valueUsd ?? 0)));
  ...
  for (const r of swapRows) {
    ...
    activity.push({
      sig: r.signature, ts: Number(r.confirmed_at ?? r.created_at), type: r.side, status, source: "apeme", side: r.side, mint: tokenMint,
      symbol: r.symbol ?? tm?.symbol ?? null, image: tm?.image ?? null, stockSymbol: tm?.kind === "meme" ? tm.quote_symbol : null,
      amount: tokenRaw ? Number(tokenRaw) / 10 ** decimals * (tm?.kind === "stock" ? Number(tm.multiplier ?? 1) : 1) : 0,
      quote: r.side === "buy" ? Number(r.in_raw) / 1e6 : r.out_raw == null ? null : Number(r.out_raw) / 1e6,
      usd: round(r.side === "buy" ? r.in_usd : r.out_usd), feeUsd: round(r.fee_usd), from: null, error: r.error,
    });
  }
  for (const d of transfers) {
    if (seen.has(d.sig)) continue;
    seen.add(d.sig);
    activity.push({ sig: d.sig, ts: d.ts, type: d.direction === "in" ? "deposit" : "withdraw", status: "confirmed", source: "chain", side: null, mint: USDC_MINT, symbol: "USDC", image: USDC_LOGO, stockSymbol: null, amount: d.amount, quote: null, usd: round(d.amount), feeUsd: null, from: d.from, error: null });
  }
  for (const r of tradeRows) {
    if (seen.has(r.signature)) continue;
    const quote = Number(r.quote_raw) / 10 ** r.quote_decimals;
    activity.push({
      sig: r.signature, ts: Number(r.block_time), type: r.side, status: "confirmed", source: "chain", side: r.side, mint: r.token_mint, symbol: r.symbol, image: r.image, stockSymbol: r.stock_symbol,
      amount: Number(r.base_raw) / 10 ** r.decimals, quote, usd: round(quote * (r.quote_usd ?? r.stock_price_usd ?? NaN)), feeUsd: null, from: null, error: null,
    });
  }
  activity.sort((a, b) => b.ts - a.ts);

  return {
    address, totalUsd: round(cashUsd + solUsd + stocksUsd + memesUsd)!, cashUsd: round(cashUsd)!, solUsd: round(solUsd)!, stocksUsd: round(stocksUsd)!, memesUsd: round(memesUsd)!,
    costUsd: round(costUsd)!, pnlUsd: round(pnlUsd)!, realizedUsd: round(realizedUsd)!, pendingSwaps,
    holdings, activity: activity.slice(0, activityLimit), asOf: Math.floor(Date.now() / 1000),
  };
}
