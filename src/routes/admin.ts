...
admin.post("/stocks/:mint", async (c) => {
  ...
  const next = { excluded: b.excluded ?? cur?.excluded ?? false, category: b.category === undefined ? cur?.category ?? null : b.category, tags: b.tags ?? cur?.tags ?? [], note: b.note ?? cur?.note ?? null };
  await sql`INSERT INTO stock_config (mint, excluded, category, tags, note, updated_at) VALUES (${mint.data}, ${next.excluded}, ${next.category}, ${next.tags}, ${next.note}, ${now})
            ON CONFLICT (mint) DO UPDATE SET excluded = ${next.excluded}, category = ${next.category}, tags = ${next.tags}, note = ${next.note}, updated_at = ${now}`;
  const [row] = await sql`UPDATE stocks SET excluded = ${next.excluded}, tags = ${next.tags}, category = COALESCE(${next.category}, category) WHERE mint = ${mint.data} RETURNING mint, symbol, category, excluded, tags`;
