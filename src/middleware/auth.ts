...
export const requireAuth: MiddlewareHandler<{ Bindings: Env; Variables: AuthVars }> = async (c, next) => {
  ...
  const token =
    c.req.header("privy-id-token")?.trim() ||
    c.req
  ...
  if (!privy.wallets.length) {
    const acc = await fetchUserAccounts(c.env, privy.id);
    if (acc) {
      privy.wallets = acc.wallets;
      privy.email ??= acc.email;
    }
  }
  const { user, wallets, settings } = await ensureUser(
    c.get("sql"),
    privy,
    Number(c.env.INVITES_PER_USER ?? 5),
  );
  c.set("privy", privy);
  c.set("user", user);
  c.set("wallets", wallets);
