...
const sha = async (s: string) =>
  Array.from(new Uint8Array(await crypto.subtle.digest("SHA-256", new TextEncoder().encode(s.toLowerCase()))))
    .map((b) => b.toString(16).padStart(2, "0"))
    .join("");

export const shapeSettings = (s: SettingsRow) => ({
  slippageBps: s.slippage_bps,
  quickBuyUsd: pgArr(s.quick_buy_usd).map(Number),
  quickSellPct: pgArr(s.quick_sell_pct).map(Number),
  priority: s.priority as "normal" | "fast" | "turbo",
  confirmBeforeTrade: s.confirm_before_trade,
  hideDust: s.hide_dust,
});
export const shapeWallet = (w: WalletRow) => ({
  address: w.address,
  chain: w.chain as "solana" | "evm",
  hdIndex: w.hd_index,
  label: w.label,
...
export async function ensureUser(
...
): Promise<{ user: UserRow; wallets: WalletRow[]; settings: SettingsRow }> {
  const t = now();
  ...
  if (!user) {
    for (let i = 0; i < 5 && !user; i++) {
      ...
      try {
        ...
        if (!/duplicate key/.test((e as Error).message)) throw e;
      }
    }
  ...
  for (const w of p.wallets) {
    ...
    await accountRepo.addWallet(sql, {
      address: w.address,
      userId: user.id,
      chain: w.chain,
      idx,
      label: idx === 0 ? "Main" : `Account ${idx + 1}`,
      isDefault: idx === 0,
      t,
    });
    idx++;
...
export async function redeemInvite(sql: Sql, userId: string, code: string) {
  ...
  if (c.uses >= c.max_uses || (c.expires_at && c.expires_at < t))
    throw new HttpError(410, "invite code used up or expired");
...
export async function referrals(sql: Sql, user: UserRow) {
  ...
  return {
    ...
    link: `https://tradestonks.fun/i/${user.referral_code}`,
    invitesLeft: code ? Math.max(0, code.max_uses - code.uses) : 0,
    referred: referred.map((r) => ({
      userId: r.id,
      handle: r.handle,
      joinedAt: Number(r.joined_at),
      volumeUsd: r.volume_usd == null ? 0 : Number(r.volume_usd),
    })),
    earnedUsd: Number(earn?.earned ?? 0),
    claimableUsd: Number(earn?.claimable ?? 0),
    payouts: (await accountRepo.payouts(sql, user.id)).map((p) => ({
      signature: p.paid_signature,
      amountUsd: Number(p.amount),
      paidAt: Number(p.paid_at),
    })),
  };
}
...
export async function claimReferrals(env: Env, sql: Sql, user: UserRow, wallets: WalletRow[]) {
  ...
  if (total < MIN_CLAIM_USD)
    throw new HttpError(422, `claimable $${total.toFixed(2)} is under the $${MIN_CLAIM_USD} minimum`);
  const dest =
    wallets.find((w) => w.is_default && w.chain === "solana") ?? wallets.find((w) => w.chain === "solana");
  if (!dest) throw badRequest("no solana wallet to pay to");
  const gas = gasKeypair(env);
  const conn = connection(env);
  const usdc = new PublicKey(USDC_MINT),
    to = new PublicKey(dest.address);
  ...
  try {
    ...
    throw new HttpError(
      503,
      /insufficient|0x1\b/i.test((e as Error).message)
        ? "payout wallet is being topped up, try again later"
        : `payout failed: ${(e as Error).message.slice(0, 120)}`,
    );
  }
  await accountRepo.markPaid(
    sql,
    rows.map((r) => r.id),
    sig,
    now(),
...
export async function updateSettings(
  ...
  const row = (
    await accountRepo.updateSettings(sql, userId, {
      slippageBps: b.slippageBps ?? cur.slippage_bps,
      buyCsv: (b.quickBuyUsd ?? pgArr(cur.quick_buy_usd).map(Number)).join(","),
      sellCsv: (b.quickSellPct ?? pgArr(cur.quick_sell_pct).map(Number)).join(","),
      priority: b.priority ?? cur.priority,
      confirm: b.confirmBeforeTrade ?? cur.confirm_before_trade,
      hideDust: b.hideDust ?? cur.hide_dust,
    })
  )[0];
...
export async function mintAdminCodes(
  sql: Sql,
  count: number,
  maxUses: number,
  label: string | null,
