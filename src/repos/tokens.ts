...
export type Sort =
  "volume" | "new" | "mcap" | "vol5m" | "vol1h" | "vol24h" | "txns1h" | "progress" | "change1h" | "change24h";
...
const sortExpr = (sql: Sql, sort: Sort) => {
  switch (sort) {
    ...
    case "mcap":
      return sql`coalesce(st.mcap_usd,0)`;
    case "vol5m":
      return sql`coalesce(st.vol_5m_usd,0)`;
    case "vol1h":
      return sql`coalesce(st.vol_1h_usd,0)`;
    case "txns1h":
      return sql`(coalesce(st.buys_1h,0) + coalesce(st.sells_1h,0))`;
    case "progress":
      return sql`coalesce(st.progress_pct,0)`;
    case "change1h":
      return sql`coalesce(st.change_1h,0)`;
    case "change24h":
      return sql`coalesce(st.change_24h,0)`;
    default:
      return sql`coalesce(st.vol_24h_usd,0)`; // volume, vol24h
  }
};
export const defaultSort = (column?: Column): Sort =>
  column === "new"
    ? "new"
    : column === "graduating"
      ? "progress"
...
const columnWhere = (sql: Sql, column: Column | undefined, now: number) => {
  switch (column) {
    case "new":
      return sql`AND t.phase = 'curve' AND t.created_at > ${now - NEW_WINDOW}`;
    case "graduating":
      return sql`AND t.phase = 'curve' AND st.progress_pct >= ${GRADUATING_PCT}`;
    case "graduated":
      return sql`AND t.phase = 'graduated'`;
    default:
      return sql``;
  }
};
...
const filterWhere = (sql: Sql, f: TokenFilters, now: number) => {
  ...
  if (f.minTxns1h != null)
    parts.push(sql`AND (coalesce(st.buys_1h,0) + coalesce(st.sells_1h,0)) >= ${f.minTxns1h}`);
  ...
  if (f.launchpad) {
    const l = f.launchpad.split(",").filter((x) => LAUNCHPADS.includes(x));
    if (l.length) parts.push(sql`AND t.launchpad IN ${sql(l)}`);
  } // array params 500 through Hyperdrive; IN list binds scalars
  if (f.dexPaid === 1) parts.push(sql`AND t.dex_paid`);
  if (f.social === 1)
    parts.push(sql`AND (t.website IS NOT NULL OR t.twitter IS NOT NULL OR t.telegram IS NOT NULL)`);
  if (f.q) {
    const q = f.q.trim();
    if (q)
      parts.push(
        sql`AND (t.symbol ILIKE ${"%" + q + "%"} OR t.name ILIKE ${"%" + q + "%"} OR t.mint LIKE ${q + "%"})`,
      );
  }
...
export const tokensRepo = {
  ...
  byMint: async (sql: Sql, mint: string) =>
    (
      await sql<TokenRow[]>`
    SELECT ${cols(sql)} FROM tokens t LEFT JOIN token_stats st ON st.token_mint = t.mint WHERE t.mint = ${mint}`
    )[0] ?? null,
  // Token joined with its quote stock: one round trip for the token page header and the trades tape.
  withStock: async (sql: Sql, mint: string) =>
    (
      await sql<(TokenRow & { stock: StockRow })[]>`
