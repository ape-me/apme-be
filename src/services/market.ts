...
const RANGE = { "5m": { span: 300, step: 5 }, "15m": { span: 900, step: 15 }, "1h": { span: 3600, step: 60 }, "1d": { span: 86400, step: 120 }, "1w": { span: 604800, step: 900 }, "1m": { span: 2592000, step: 3600 } } as const;
...
export const market = {
  ...
  history: async (sql: Sql, mint: string, range: z.infer<typeof HistoryRange>): Promise<z.infer<typeof HistoryResponse>> => {
    ...
    const rows = step < 60 ? await marketRepo.ticks(sql, mint, from, step) : await marketRepo.history(sql, mint, from, step);
