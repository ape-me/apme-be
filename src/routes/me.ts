...
me.patch("/", async (c) => {
  const b = parse(
    z.object({ handle: Handle.optional(), avatarUrl: z.string().url().max(500).nullable().optional() }),
    await c.req.json(),
  );
...
me.post("/referrals/claim", async (c) =>
  c.json(await claimReferrals(c.env, c.get("sql"), c.get("user"), c.get("wallets"))),
);
...
me.patch("/wallets/:address", async (c) => {
  ...
  const b = parse(
    z.object({ label: z.string().min(1).max(24).optional(), isDefault: z.literal(true).optional() }),
    await c.req.json(),
  );
  const sql = c.get("sql");
